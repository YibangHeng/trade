...
struct CUTTradingAccountField
{
	...
	///它席保证金可用余额(只同步负值)
	TUTMoneyType	CreditAvailableOtherSeat;
	///保证金可用余额出入金
	TUTMoneyType	CreditAvailableDeposit;
	///它席担保品买入可用资金(只同步负值)
	TUTMoneyType	CreditCollBuyAvailOtherSeat;
};
///持仓
...
struct CUTInstrumentField
{
	...
	///币种代码
	TUTCurrencyIDType	CurrencyID;
	///市价单最小买下单单位;个股期权表示参数1放大10000倍
	TUTVolumeType	MinMarketBuyVolume;
	///市价单最小卖下单单位;个股期权表示参数2放大10000倍
...
///手工出入金
struct CUTSyncCreditParamField
{
	///投资者代码
	TUTInvestorIDType	InvestorID;
	///入金金额
	...
	TUTBoolType	IsForce;
	///两融出入金类型
	TUTCreditDepositTypeType	CreditDepositType;
};
///报单
...
///UDP会话引用
struct CUTUDPSessionRefField
{
	///请求编号
	TUTRequestIDType	RequestID;
	///前置编号
	TUTFrontIDType	FrontID;
	///会话编号
	TUTSessionIDType	SessionID;
