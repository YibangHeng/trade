...
TEST_CASE("Serializing and deserializing messages", "[Serializer]")
{
    SECTION("Message with message_id == 0 and empty message")
    {
    ...
    SECTION("Message with message_id == 0 and non-empty message")
    {
    ...
    SECTION("Message with an out-of-range message_id")
    {
        trade::types::UnixSig unix_sig;
        unix_sig.set_sig(2);
...
TEST_CASE("Communication between RRServer and RRClient", "[RRServer/RRClient]")
{
...
TEST_CASE("Communication with UDP multicast", "[MCServer/MCClient]")
{
    SECTION("Sending and receiving messages via IP multicast")
    {
